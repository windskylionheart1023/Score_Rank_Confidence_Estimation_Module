...
class Hypothesis:
    ...
    dec_out_list: List[List[float]] = field(default_factory=list)
    ...
    def default_beam_search(self, enc_out: torch.Tensor) -> List[Hypothesis]:
        ...
        for t, enc_out_t in enumerate(enc_out):
            ...
            while True:
                ...
                scores_dict = {str(index.item()): value.item() for index, value in zip(top_k.indices, top_k.values)}
                ...
                kept_hyps.append(
                    Hypothesis(
                        ...
                        scores_list=max_hyp.scores_list,
                        time_steps=max_hyp.time_steps,
                        dec_out_list=max_hyp.dec_out_list,
                    )
                )
                ...
                for logp, k in zip(*top_k):
                    ...
                    hyps.append(
                        Hypothesis(
                            ...
                            dec_state=state,
                            lm_state=lm_state,
                            scores_list=max_hyp.scores_list + [scores_dict],
                            time_steps=max_hyp.time_steps[:] + [t],
                            dec_out_list=max_hyp.dec_out_list + [dec_out.tolist()],
                        )
                    )
