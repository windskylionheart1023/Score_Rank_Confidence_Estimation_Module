...
def inference(
    ...
    if (speech2text.asr_model.use_transducer_decoder) or (speech2text.asr_model.decoder is None):
        attention_based = False
    else:
        attention_based = True

    if attention_based:
        hs_list = {key: [] for key in range(1, nbest + 1)}
    # scores_list_before_pre_beam = {key: [] for key in range(1, nbest + 1)}

    if speech2text.beam_search_transducer:
        time_step_list = {key: [] for key in range(1, nbest + 1)}
        dec_out_list = {key: [] for key in range(1, nbest + 1)}
        ...
        for keys, batch in loader:
            ...
            if enh_s2t_task or multi_asr:
                ...
                for n, (text, token, token_int, hyp) in zip(
                    ...
                    if attention_based:
                        hs_list[n].append((key, [t.tolist() for t in hyp.hs]))
                    # scores_list_before_pre_beam[n].append(
                    #     (key, hyp.scores_list_before_pre_beam)
                    # )
                    if speech2text.beam_search_transducer:
                        time_step_list[n].append((key, hyp.time_steps))
                        dec_out_list[n].append((key, hyp.dec_out_list))
    ...
    if (record_scores := True):
        for n in range(1, nbest+1):
            ...
            if attention_based:
                hs_file_path = f"{output_dir}/{n}best_recog/hs_list.json"
                with open(hs_file_path, "w") as file:
                    json.dump(hs_list[n], file)
            
            if speech2text.beam_search_transducer:
                time_step_file_path = f"{output_dir}/{n}best_recog/time_step_list.json"
                with open(time_step_file_path, "w") as file:
                    json.dump(time_step_list[n], file)
                dec_out_file_path = f"{output_dir}/{n}best_recog/dec_out_list.json"
                with open(dec_out_file_path, "w") as file:
                    json.dump(dec_out_list[n], file)
