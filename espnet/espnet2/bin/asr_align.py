...
class CTCSegmentation:
    ...
    @torch.no_grad()
    def get_lpz(self, speech: Union[torch.Tensor, np.ndarray], return_hs_enc: bool = False):
        ...
        if return_hs_enc:
            lpz, hs = self.ctc.log_softmax(enc, return_hs=True)
            lpz = lpz.detach()
        else:
    ...
    def __call__(
        ...
        if return_hs_enc:
            lpz, hs, enc = self.get_lpz(speech, return_hs_enc=True)
        else:
